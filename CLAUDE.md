...
3. **MCP configurator** (`src/mcp/configurator.ts`) writes 100% on-device MCP servers (`token-cache`, `headroom`, `codegraph`) into `.vscode/settings.json`, `.agents/mcp_config.json`, and `~/.claude.json`. It actively **deletes** any `rtk` (which uses hooks) or third-party cloud entries like `context7` to enforce 100% on-device zero cloud leakage.
